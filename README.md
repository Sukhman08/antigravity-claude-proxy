...
| `/v1/chat/completions` | POST   | **OpenAI Chat Completions API** (compatibility layer)                 |
...
The proxy now supports the **OpenAI Chat Completions API** format, allowing you to use OpenAI-compatible clients and libraries with Claude and Gemini models.
...
client = OpenAI(
    ...
    api_key="test"  # Any non-empty string works
)
...
### Including Thinking Output

For thinking models, you can include the model's reasoning in the response:

```bash
# Via header
curl http://localhost:8080/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "X-Include-Thinking: true" \
  -d '{"model": "claude-sonnet-4-5-thinking", "messages": [{"role": "user", "content": "Solve 2+2"}]}'

# Via query parameter
curl "http://localhost:8080/v1/chat/completions?include_thinking=true" \
  -H "Content-Type: application/json" \
  -d '{"model": "claude-sonnet-4-5-thinking", "messages": [{"role": "user", "content": "Solve 2+2"}]}'
```

Thinking content is included as `<thinking>...</thinking>` blocks in the response text.
...
| Image inputs | ✅ Full support |
| Thinking models | ✅ With optional output |

### Unsupported OpenAI Features

These OpenAI-specific features are not supported by the backend:
...
These parameters are silently ignored if provided.
