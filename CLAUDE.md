...
│   ├── openai-request-converter.js   # OpenAI → Anthropic conversion
│   ├── openai-response-converter.js  # Anthropic → OpenAI conversion
│   └── openai-streaming.js     # OpenAI streaming format converter
│
└── utils/                      # Utilities
...
- **src/server.js**: Express server exposing Anthropic-compatible endpoints (`/v1/messages`, `/v1/chat/completions`, `/v1/models`, `/health`, `/account-limits`) and mounting WebUI
...
- **src/format/**: Format conversion between Anthropic, Google Generative AI, and OpenAI formats
  - `openai-request-converter.js`: Converts OpenAI Chat Completions requests to Anthropic format
  - `openai-response-converter.js`: Converts Anthropic responses to OpenAI format
  - `openai-streaming.js`: Converts Anthropic SSE events to OpenAI streaming format
...
- Endpoint: `POST /v1/chat/completions` provides OpenAI Chat Completions API compatibility
- Request conversion: `convertOpenAIToAnthropic()` transforms OpenAI format to Anthropic format
- Response conversion: `convertAnthropicToOpenAI()` transforms responses back to OpenAI format
- Streaming: `writeOpenAIStream()` converts Anthropic SSE events to OpenAI streaming chunks
- System messages: Extracted from messages array and converted to Anthropic `system` field
- Tools: Unwrapped from `{type: "function", function: {...}}` to Anthropic format
- Tool results: Converted from `{role: "tool", tool_call_id}` to Anthropic `tool_result` blocks
- Thinking: Optionally included via `X-Include-Thinking: true` header or `?include_thinking=true` query param
- Unsupported OpenAI features (ignored): `logprobs`, `n`, `response_format`, `seed`, `frequency_penalty`, `presence_penalty`
