 ...
 *
 * Provides OpenAI API compatibility layer by converting requests/responses
...
app.post('/v1/chat/completions', async (req, res) => {
    try {
        ...
        logger.info(`[API] OpenAI request for model: ${anthropicRequest.model}, stream: ${!!anthropicRequest.stream}`);

        // Check if thinking content should be included (via custom header or query param)
        const includeThinking = req.headers['x-include-thinking'] === 'true' ||
                                req.query.include_thinking === 'true';
        ...
        if (anthropicRequest.stream) {
            ...
            try {
                ...
                await writeOpenAIStream(res, anthropicStream, anthropicRequest.model, includeThinking);
                res.end();
                ...
                res.write(`data: ${JSON.stringify({
                    error: {
                        message: errorMessage,
                        type: errorType === 'authentication_error' ? 'invalid_api_key' : 'api_error',
                        code: errorType
                    }
                })}\n\n`);
                res.write('data: [DONE]\n\n');
            ...
            const openaiResponse = convertAnthropicToOpenAI(anthropicResponse, anthropicRequest.model, includeThinking);
        ...
        logger.warn(`[API] Returning OpenAI error response: ${statusCode} ${errorType} - ${errorMessage}`);
